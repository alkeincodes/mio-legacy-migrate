...
| Auth pages (login, register, onboarding) | `resources/modules/hub/js/Pages/Login.vue:96-175`, `Register.vue:196-300`, `Onboarding.vue:123-216`, `sass/_common.scss:116-135`, `sass/_auth.scss` | one split layout; panel background = page `settings.background` else theme `pages.<type>.background` (register, onboarding fall back to login); `variant-default` = 5% wash of the text colour, dark hubs fill with secondary under white; `variant-image` is white text, no tint; side from `hub.meta.<login|registration|onboarding>.sections`; logo size `pages.<type>.logoSize` (150) |
...
| Auth brand panel | `src/lib/auth/prepare-panel-region.ts`, `src/components/auth/branded-auth-shell.tsx`, `derive-auth-shell-props.ts` | login, register, forgot, reset, magic link and onboarding all render from the `login` page (register from `register`); first root child with `settings.slot: "brand-panel"` gives `surface` (section vocabulary, `ink` auto/light/dark), `side`, `logoSize` (number 16..320 or normal/hero); allowed kinds stack/headline/text/image/divider/countdown; no page: solid primary panel; `auth_logo_url` then `logo_url` then hub name; slugs `login`/`register` are reserved (backend `service.py:114`), pages are found by type |
