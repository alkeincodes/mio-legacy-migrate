...
program
  ...
  .option('--shots', 'capture the legacy/V3 contact sheet (off by default until the assets have landed)', false)
  .action(async (opts: { run: string; profile: string; plan: string; milestone: string; shots: boolean }) => {
